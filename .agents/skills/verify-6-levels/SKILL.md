...
definidos no workflow Boris Cherny + Continuous Codex.
...
echo "=== NÍVEL 1: LINT ===" && \
find . -name "*.py" -path "./.Codex/*" -exec python3 -m py_compile {} \; && \
echo "PASSED"
...
grep -r "sk-" .Codex/ --include="*.json" && echo "WARNING: API keys found" || echo "PASSED"
```
