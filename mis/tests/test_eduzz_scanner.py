"""Tests for EduzzScanner — fallback-only scanner (SCAN-BR-01).
...
from mis.scanners.eduzz import EduzzScanner
...
async def test_happy_path():
    ...
    async with EduzzScanner() as scanner:
...
async def test_upsert_no_duplicates(tmp_path):
    ...
    db.execute(
        ...
        "VALUES (4, 'Eduzz', 'eduzz', 'https://eduzz.com', '2026-01-01T00:00:00Z')"
    )
    db.execute(
    ...
    count_before = list(db.execute(
        "SELECT COUNT(*) FROM products WHERE platform_id=?", [EDUZZ_PLATFORM_ID]
    ))[0][0]
