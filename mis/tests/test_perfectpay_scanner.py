"""Tests for PerfectPayScanner — fallback-only scanner (SCAN-BR-03).

6 tests (structure identical to EduzzScanner tests with PERFECTPAY_PLATFORM_ID=6):
...
from mis.scanners.perfectpay import PerfectPayScanner
...
async def test_happy_path():
    ...
    async with PerfectPayScanner() as scanner:
...
async def test_fallback_selector():
    """[] returned regardless of platform_slug — pure fallback."""
    async with PerfectPayScanner() as scanner:
...
async def test_upsert_no_duplicates(tmp_path):
    ...
    db.execute(
        ...
        "VALUES (6, 'PerfectPay', 'perfectpay', 'https://app.perfectpay.com.br', '2026-01-01T00:00:00Z')"
    )
    db.execute(
    ...
    count_before = list(db.execute(
        "SELECT COUNT(*) FROM products WHERE platform_id=?", [PERFECTPAY_PLATFORM_ID]
    ))[0][0]
    ...
    if not result:
        mark_stale(db, PERFECTPAY_PLATFORM_ID, 1)
...
async def test_is_stale(tmp_path):
    ...
    product = Product(
        external_id="perfectpay-test-001",
        title="Produto PerfectPay Teste",
        url="https://app.perfectpay.com.br/produto-teste",
        platform_id=PERFECTPAY_PLATFORM_ID,
        niche_id=1,
        rank=1,
        price=97.0,
    )
    ...
    row = list(db.execute(
        ...
        [PERFECTPAY_PLATFORM_ID, "perfectpay-test-001"],
    ))[0]
