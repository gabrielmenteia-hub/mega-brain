...
| Arquivo | Propósito |
|---------|-----------|
| `.Codex/scripts/build-complete-index.py` | Constrói índice nome→TAG da planilha |
| `.Codex/scripts/tag-inbox-v2.py` | Faz matching e renomeia arquivos |
| `.Codex/scripts/revert-tags.py` | Remove TAGs incorretas (rollback) |
| `.Codex/mission-control/PLANILHA-INDEX.json` | Índice com 915+ entradas |
| `.Codex/mission-control/TAG-MAPPING-V2.json` | Relatório do último matching |
| `.Codex/mission-control/SPREADSHEET-SCHEMA.json` | Schema das abas da planilha |
...
python3 .Codex/scripts/build-complete-index.py
```

### Passo 2: Executar Matching (preview)
```bash
python3 .Codex/scripts/tag-inbox-v2.py --threshold=0.65
```
...
```bash
python3 .Codex/scripts/tag-inbox-v2.py --threshold=0.65 --execute
```

### Rollback (se necessário)
```bash
python3 .Codex/scripts/revert-tags.py
```
