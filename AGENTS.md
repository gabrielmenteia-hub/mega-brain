...
Plans MUST be saved to `docs/plans/` (not ~/.Codex/plans/).
...
|-------|---------|------------|
| L1 (Community) | core/, agents/conclave, .Codex/, bin/, docs/ | Tracked (npm package) |
...
| `ANTHROPIC_API_KEY` | N/A with Codex | Not needed |

## Hooks System

20+ active hooks in `.Codex/hooks/` (Python 3, stdlib + PyYAML only).
...
(Moved out of `.Codex/rules/` to prevent auto-loading into context)
...
- Only 2 AGENTS.md files are valid: root `AGENTS.md` and `.Codex/AGENTS.md` (this file)
- NEVER create AGENTS.md in data or code subdirectories
- Agent memory lives in `.Codex/jarvis/` and `.Codex/skills/`
